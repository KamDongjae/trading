...
# ---- [정확도 개선 제안 PDF 반영] ----------------------------------------
# ①④ 상위타임프레임 EMA 정배열 확인용 — [2차 리뷰 반영] 15분/30분(진입타이밍)과 주봉(장기
#    추세)까지 확장. 코인당 fetch가 늘어 실행시간이 더 걸리지만 "무거워도 된다" 전제 유지.
HTF_INTERVALS = [("minute15", "15m"), ("minute30", "30m"), ("minute240", "4H"),
                  ("day", "1D"), ("week", "1W")]
...
# [2차 리뷰 #3] 거래량 심화 분석 — RVOL(상대거래량), VWAP 이탈률, 거래량 스파이크
RVOL_PERIOD = 20                # 최근 20봉 평균 대비 현재 거래량 배율
VWAP_PERIOD = 20                # 롤링 VWAP 기간 (거래소가 세션 개념이 없어 롤링으로 대체)
VOLUME_SPIKE_MULT = 2.0         # 평균 대비 이 배율 넘으면 "거래량 스파이크"로 표시
...
# [2차 리뷰 #3,#5] volume(RVOL/VWAP), btc_influence(BTC 변동성/거래량/RSI/펀딩) 추가.
DEFAULT_WEIGHTS = {
    "htf": 0.20,          # 상위 타임프레임(15m~1W) EMA 정배열 방향
    "obv": 0.12,          # OBV 최근 기울기(자금 유입/유출)
    "cmf": 0.12,          # CMF 부호(매집/분산)
    "bb_macd": 0.16,      # BB 극단 위치 + MACD_Hist 반전 컨플루언스
    "funding": 0.08,      # 펀딩비 역방향(쏠림 과열 시 반대 신호)
    "dominance": 0.12,    # BTC 도미넌스 상승/하락에 따른 알트 가중치
    "volume": 0.10,       # RVOL/VWAP 이탈 — 거래량 뒷받침 없는 신호 감점
    "btc_influence": 0.10,  # BTC 자체 변동성/거래량/RSI/펀딩 기반 알트 종속 신호
}
WEIGHTS_STATE_PATH = os.path.join(SCRIPT_DIR, "grok_ensemble_weights.json")

# [2차 리뷰 #2] 시장 국면별 가중치 배율 — 국면마다 어떤 신호가 더 신뢰할만한지 다르다:
#   상승/하락장(추세장): 추세추종형 신호(htf, obv, btc_influence)를 더 신뢰
#   횡보장: 평균회귀형 신호(cmf, bb_macd)를 더 신뢰, 추세신호는 덜 신뢰(휩쏘 방지)
# rule_score 계산 직전에 base weights에 곱한 뒤 합이 1이 되도록 재정규화한다.
REGIME_WEIGHT_MULTIPLIERS = {
    "STRONG_UPTREND":   {"htf": 1.4, "obv": 1.3, "btc_influence": 1.3, "cmf": 0.7, "bb_macd": 0.7},
    "UPTREND":          {"htf": 1.2, "obv": 1.15, "btc_influence": 1.15, "cmf": 0.85, "bb_macd": 0.85},
    "STRONG_DOWNTREND": {"htf": 1.4, "obv": 1.3, "btc_influence": 1.3, "cmf": 0.7, "bb_macd": 0.7},
    "DOWNTREND":        {"htf": 1.2, "obv": 1.15, "btc_influence": 1.15, "cmf": 0.85, "bb_macd": 0.85},
    "NEUTRAL":          {"htf": 0.75, "obv": 0.85, "btc_influence": 0.85, "cmf": 1.3, "bb_macd": 1.3},
}

# ⑧ 워크포워드 백테스트 + 자동 가중치 최적화 — 단, 이번 실행에서 fetch한 시계열(빗썸
#    시간봉 200개)만으로 재구성 가능한 신호(obv/cmf/bb_macd/volume)만 최적화 대상이다. htf/
#    funding/dominance/btc_influence는 "현재 시점 스냅샷" 위주라 과거 시계열을
#    안 쌓아서 워크포워드로 최적화할 수 없다 — 이 나머지는 DEFAULT_WEIGHTS(x국면배율)
#    값을 그대로 쓰고, obv/cmf/bb_macd/volume 비중만 과거 데이터로 재배분한다. 정직하게 밝히는
#    한계.
# [2차 리뷰 #1 데이터 누수 방지] 이번 실행에서 새로 학습한 가중치는 바로 이번 추천에
#    쓰지 않고 WEIGHTS_STATE_PATH에 저장만 해서 "다음" 실행부터 적용된다 — 아래
#    main()에서 load_weights()가 실행 맨 앞(이전 실행 결과)에서 한 번만 일어나고,
#    run_walk_forward_backtest()의 결과는 save_weights()로 저장만 하고 이번 실행의
#    rule_score 계산에는 재사용하지 않는다.
...
BACKTEST_OPTIMIZABLE = ["obv", "cmf", "bb_macd", "volume"]
...
# [2차 리뷰 #9] 히트레이트뿐 아니라 손익비(Profit Factor)/평균수익 대비 변동성(Sharpe 유사
#    지표)/최대낙폭(MDD)까지 매 실행마다 계산해서 대시보드로 출력한다. Precision/Recall은
...
#    이라 스레드풀만으로도 크게 빨라진다. 차트 그리기(matplotlib)는 스레드 안전성 때문에
#    fetch 다 끝난 뒤 메인 스레드에서 순차 처리.
MAX_PARALLEL_FETCH_WORKERS = 8

# [2차 리뷰 #7] Grok에게 원본 CSV 20행 대신, 코인당 마지막 시점 핵심 피처만 압축한 JSON
#    스냅샷을 전달한다 (차트 이미지는 시각패턴 인식에 필요하니 그대로 유지, PDF 리포트
#    텍스트도 보조참고용으로 축소해서 유지).
CONDENSED_JSON_MAX_CHARS = 25000
# ---------------------------------------------------------------------------
# ===========================================
...
def load_data(coin):
    ...
    # [2차 리뷰 #3] 거래량 심화: RVOL(상대거래량), 롤링 VWAP 이탈률(%), 거래량 스파이크 플래그
    vol_avg = df['volume'].rolling(RVOL_PERIOD).mean()
    df['RVOL'] = df['volume'] / vol_avg.replace(0, float('nan'))
    ...
    df['Volume_Spike'] = df['RVOL'] >= VOLUME_SPIKE_MULT
...
def _apply_regime_multiplier(weights, regime):
    """[2차 리뷰 #2] 시장 국면별로 신뢰할 신호가 다르다 — 추세장에선 추세추종형(htf/obv/
    btc_influence)을, 횡보장에선 평균회귀형(cmf/bb_macd)을 더 신뢰하도록 배율을 곱한 뒤,
    전체 합이 원래와 같아지도록 재정규화한다(스케일 유지)."""
    mult = REGIME_WEIGHT_MULTIPLIERS.get(regime, {})
    adjusted = {k: v * mult.get(k, 1.0) for k, v in weights.items()}
    orig_sum = sum(weights.values()) or 1e-9
    new_sum = sum(adjusted.values()) or 1e-9
    return {k: v * (orig_sum / new_sum) for k, v in adjusted.items()}


def _volume_signal(df):
    """[2차 리뷰 #3] RVOL(상대거래량)과 VWAP 이탈률을 결합 — 거래량 뒷받침 없는 가격 이탈은
    신뢰도를 낮추고, 거래량이 실린 이탈(스파이크)은 신호를 강화한다."""
    if df is None or len(df) < 2:
        return 0.0
    last = df.iloc[-1]
    vwap_dev = last.get('VWAP_Dev_Pct', 0.0)
    rvol = last.get('RVOL', 1.0)
    vwap_dev = 0.0 if pd.isna(vwap_dev) else float(vwap_dev)
    rvol = 1.0 if pd.isna(rvol) else float(rvol)
    conviction = min(1.5, rvol) / 1.5  # RVOL 1.5배 이상이면 거래량 뒷받침 만점
    return max(-1.0, min(1.0, (vwap_dev / 5.0))) * conviction


def compute_btc_context(btc_df, btc_htf, btc_funding_oi):
    """[2차 리뷰 #5] BTC EMA 정배열뿐 아니라 BTC 자체의 모멘텀(MACD)/RSI 위치/펀딩비까지
    합쳐서 "BTC가 지금 얼마나, 어느 방향으로 알트 시장을 끌고가는지"를 하나의 방향성
    점수로 만든다. 알트코인 rule_score의 btc_influence 항목에 공통으로 쓰인다."""
    if btc_df is None or len(btc_df) < 20:
        return {"direction": 0.0, "note": "BTC 데이터 없음"}
    last = btc_df.iloc[-1]
    htf_score = btc_htf.get("score", 0.0) if btc_htf else 0.0
    macd_hist = last.get('MACD_Hist', 0.0)
    macd_sign = 1.0 if (pd.notna(macd_hist) and macd_hist > 0) else (-1.0 if pd.notna(macd_hist) and macd_hist < 0 else 0.0)
    rsi = last.get('RSI', 50.0)
    rsi_lean = 0.0 if pd.isna(rsi) else max(-1.0, min(1.0, (float(rsi) - 50) / 50))
    funding_score = 0.0
    if btc_funding_oi and btc_funding_oi.get("funding_rate") is not None:
        funding_score = max(-1.0, min(1.0, -btc_funding_oi["funding_rate"] * 200))
    direction = 0.45 * htf_score + 0.25 * macd_sign + 0.15 * rsi_lean + 0.15 * funding_score
    direction = max(-1.0, min(1.0, direction))
    adx_val = last.get('ADX', 0.0)
    adx_val = 0.0 if pd.isna(adx_val) else float(adx_val)
    return {
        "direction": direction, "adx": adx_val, "rsi": None if pd.isna(rsi) else round(float(rsi), 1),
        "funding_score": round(funding_score, 3),
        "note": f"BTC 방향점수 {direction:+.2f} (ADX {adx_val:.1f}, RSI {rsi_lean*50+50:.0f})",
    }


def compute_rule_score(df, htf, funding_oi, dominance, is_btc, weights, btc_context=None):
    """
    [정확도 개선 ⑦ 앙상블] Grok의 정성 판단과 별개로, 정량 지표만으로 -1(숏 우세)~+1(롱 우세)
    사이 rule_score를 계산한다. ADX가 낮으면(횡보) 신뢰도를 통째로 낮춰서 무추세 구간에서
    과신호를 내지 않게 한다. weights는 이미 시장국면 배율이 적용된 값이 들어온다
    (build_combined_outputs에서 _apply_regime_multiplier로 한 번만 계산해서 넘김).
    ...
    volume_score = _volume_signal(df)
    ...
    # [2차 리뷰 #5] BTC 자체 신호는 알트에만 적용(BTC가 스스로에게 영향받는다고 이중계산 안 함).
    #    BTC의 추세가 강할수록(ADX 높을수록) 알트에 대한 영향력도 커진다.
    btc_influence_score = 0.0
    if not is_btc and btc_context and btc_context.get("direction") is not None:
        btc_adx = btc_context.get("adx", 0.0) or 0.0
        btc_strength = min(1.0, btc_adx / ADX_TREND_MIN) if btc_adx else 0.3
        btc_influence_score = max(-1.0, min(1.0, btc_context["direction"] * btc_strength))
    ...
    composite = (
        ...
        weights.get("btc_influence", 0) * btc_influence_score
    )
    ...
    detail = {
        ...
        "volume_score": round(volume_score, 3), "btc_influence_score": round(btc_influence_score, 3),
...
#    (obv/cmf/bb_macd/volume만 최적화 — htf/funding/dominance/btc_influence는 "현재
#    시점 스냅샷" 위주라 과거 시계열이 없어서 고정, 위 DEFAULT_WEIGHTS 주석 참고)
# ============================================================
def _backtest_score_sample(df, idx, weights):
    ...
    volume_score = _volume_signal(sub)
...
def _build_backtest_samples(dfs_by_ticker):
    ...
    가중치와 무관한 부분(obv/cmf/bb_macd/volume 원재료)이라 샘플 추출 시 한 번만 하고,
    가중치 최적화 루프에서는 이미 계산된 시그널 값만 재조합한다(랜덤서치 250회를 매번
    지표 재계산하면 너무 느려서, 시그널 4개를 미리 뽑아 캐싱)."""
    samples = []  # list of (obv_score, cmf_score, bb_macd_score, volume_score, forward_return)
    for coin, df in dfs_by_ticker.items():
        ...
        for idx in range(BACKTEST_MIN_LOOKBACK, n - BACKTEST_HOLD_BARS, BACKTEST_STEP):
            ...
            samples.append((obv_score, cmf_score, bb_macd_score, volume_score, fwd_ret))
    return samples


def _objective(samples, w_obv, w_cmf, w_bbm, w_vol):
    ...
    for obv_s, cmf_s, bbm_s, vol_s, fwd_ret in samples:
        score = w_obv * obv_s + w_cmf * cmf_s + w_bbm * bbm_s + w_vol * vol_s
...
def run_walk_forward_backtest(dfs_by_ticker, base_weights):
    ...
    데이터만 사용, 미래 참조 없음)으로 obv/cmf/bb_macd/volume 4개 가중치를 랜덤서치로
    재배분한다. 표본이 코인당 최대 (200-66)/4 ≈ 33개 x 코인수 정도라 완전한 백테스트라기
    보단 "최근 수일간 어떤 신호 조합이 잘 맞았는지"를 매 실행마다 가볍게 재추정하는 온라인
    재보정에 가깝다 — 정직하게 그 한계를 로그에 남긴다.
    """
    ...
    optimizable_budget = max(0.05, 1.0 - fixed_sum)  # obv+cmf+bb_macd+volume이 나눠 가질 총량

    base_score = _objective(
        samples, base_weights["obv"], base_weights["cmf"], base_weights["bb_macd"], base_weights["volume"]
    )

    best = (base_weights["obv"], base_weights["cmf"], base_weights["bb_macd"], base_weights["volume"])
    best_score = base_score
    for _ in range(BACKTEST_RANDOM_ITERS):
        a, b, c, d = random.random(), random.random(), random.random(), random.random()
        s = (a + b + c + d) or 1e-9
        w_obv, w_cmf, w_bbm, w_vol = (
            (a / s) * optimizable_budget, (b / s) * optimizable_budget,
            (c / s) * optimizable_budget, (d / s) * optimizable_budget,
        )
        score = _objective(samples, w_obv, w_cmf, w_bbm, w_vol)
        if score > best_score:
            best_score = score
            best = (w_obv, w_cmf, w_bbm, w_vol)

    new_weights = dict(base_weights)
    new_weights["obv"], new_weights["cmf"], new_weights["bb_macd"], new_weights["volume"] = best
    print(f"   표본 {len(samples)}개, 목적함수 {base_score:+.3f} → {best_score:+.3f} "
          f"(obv={best[0]:.2f}, cmf={best[1]:.2f}, bb_macd={best[2]:.2f}, volume={best[3]:.2f})")
    return new_weights
...
def build_combined_outputs(tickers, rows=None, weights=None, dominance=None, regime="NEUTRAL"):
    ...
    # ---- 1단계: 코인별 캔들/HTF/펀딩·OI 병렬 fetch (네트워크 I/O 대기가 대부분이라 스레드풀 효과 큼) ----
    ...
    with ThreadPoolExecutor(max_workers=MAX_PARALLEL_FETCH_WORKERS) as pool:
        ...
        for future in as_completed(futures):
            coin = futures[future]
            try:
                c, df, htf, funding_oi = future.result()
            except Exception as e:
                print(f"     ⚠️ {coin} fetch 중 예외: {e}")
                continue
            if df is None:
                print(f"     ⚠️ {coin} 데이터 못 가져옴, 건너뜀")
                continue
            raw_by_ticker[c] = (df, htf, funding_oi)
    ...
    # ---- 3단계: 순서대로 차트/CSV/rule_score 생성 (matplotlib은 메인 스레드에서 순차 처리) ----
    csv_frames = []
    ...
    with PdfPages(combined_pdf_path) as pdf_pages:
        for coin in tickers:
            if coin not in raw_by_ticker:
                continue
            print(f"   - {coin} 차트/rule_score 생성 중...")
...
def ask_grok(indicator_pdf_path, combined_csv_path, chart_images_b64, all_tickers,
    ...
    # [2차 리뷰 #7] AI에게 너무 많은 정보를 주면 오히려 성능이 떨어질 수 있어 텍스트 분량을
    # 줄였다 — 상세 수치는 아래 JSON 스냅샷과 차트 이미지가 담당하고, 리포트는 보조 참고용.
    report_text = ""
    try:
    ...
    # [2차 리뷰 #7] 코인당 최근 20행 원본 CSV 대신, 마지막 시점의 핵심 피처만 압축한 JSON
    # 스냅샷을 전달한다. LLM은 정보가 많다고 항상 잘 판단하는 게 아니라서, 차트 이미지(시각
    # 패턴)와 역할을 분담시킨다 — 세부 흐름은 이미지로, 정량 스냅샷은 JSON으로.
    df_all = pd.read_csv(combined_csv_path)
    snapshot = {}
    keep_cols = ['RSI', 'RSI_Delta', 'MACD_Hist', 'BB_Position', 'ATR_Pct', 'ADX',
                 'CMF', 'RVOL', 'VWAP_Dev_Pct', 'chg_30m', 'rule_score',
                 'rule_confidence_scale', 'htf_4h', 'htf_1d', 'funding_rate']
    for ticker, g in df_all.groupby('ticker'):
        last = g.iloc[-1]
        row = {}
        for col in keep_cols:
            if col in last and pd.notna(last[col]):
                row[col] = round(float(last[col]), 4)
    ...
    system_prompt = (
        ...
        "indicator values for the coins currently tracked, (2) a condensed per-coin JSON snapshot "
        "(latest values only, not full history) with fields RSI, RSI_Delta, MACD_Hist, BB_Position, "
        "ATR_Pct, ADX, CMF, RVOL (relative volume vs its own 20-bar average), VWAP_Dev_Pct (% price is "
        "above/below rolling VWAP), chg_30m, MA_Align (1=bullish MA5>MA20>MA60 stack, -1=bearish stack, "
        "0=mixed), rule_score/rule_confidence_scale, htf_4h/htf_1d (1=bullish EMA stack, -1=bearish, "
        "0=mixed on that timeframe), and funding_rate, and (3) candlestick chart images per coin (use "
        "these for visual pattern/divergence reading — the JSON only has the latest snapshot, not the "
        "full path). This is NOT a pre-filtered shortlist — it is the full set of coins currently "
        "tracked, with no prior ranking or bias applied. You must judge every coin yourself from the "
        "raw data and decide its direction (if any). "
        ...
        "cautious when they conflict (e.g. bullish 1H stack but bearish 1D).\n"
        "2) Divergence & BB Cross: look for divergence between price action and RSI across the chart "
        "images. Give extra weight to tickers where BB_Position is near 1 (pressing the upper band) or "
        "near 0 (pressing the lower band) AND MACD_Hist shows a reversal signal (e.g. flipping sign or "
        "shrinking against the prevailing trend) at the same time — that confluence is stronger than "
        "either alone.\n"
        f"3) {low_liq_note}\n"
        ...
        "currently is; very high ATR_Pct means wider, less reliable swings, so demand a clearer setup.\n"
        "5) Volume Confirmation: RVOL above ~1.5 means the current move has real volume behind it — "
        "treat price moves with low RVOL as weaker/more suspect regardless of how clean the indicators "
        "look. VWAP_Dev_Pct tells you whether price is already extended above/below its recent volume-"
        "weighted average price; a large move already far from VWAP with low RVOL is more likely to "
        "mean-revert than continue.\n"
        "6) Ensemble Signal: rule_score (range -1 to +1) is an independent quantitative ensemble score "
        "computed from multi-timeframe trend alignment, OBV/CMF money flow, BB+MACD confluence, "
        "funding-rate crowding, BTC-dominance regime, RVOL/VWAP, and BTC's own momentum/RSI/funding "
        "(for altcoins) — NOT from an LLM, and its internal weighting already adapts to the current "
        "market regime. Treat it as a second opinion: when your own read and rule_score agree (same "
    ...
    content = [
        ...
        {"type": "text", "text": "=== Condensed per-coin indicator snapshot (JSON, latest values only) ===\n" + csv_text},
    ]
...
def compute_performance_stats(log, lookback=HIT_RATE_LOOKBACK):
    """[2차 리뷰 #9] 승률뿐 아니라 손익비(Profit Factor)/거래당 평균수익 대비 변동성(Sharpe
    유사 지표)/최대낙폭(MDD)까지 계산해서 어떤 조합이 실제 도움이 되는지 판단할 근거를
    남긴다. Precision/Recall은 "우리가 낸 추천이 다 맞았나"만 볼 수 있고 비교할 음성
    (negative) 후보군이 없어 엄밀하게는 계산 불가 — win_rate로 갈음한다(정직한 한계).
    """
    resolved = [e for e in log if e.get("status") in ("won", "lost")][-lookback:]
    if not resolved:
        return None
    pnls = [e.get("pnl_pct", 0.0) for e in resolved]
    ...
    # 최대낙폭: 추천 시점 순서대로 pnl_pct를 누적한 가상 자산곡선의 고점 대비 최대 하락폭
    ...
    return {
        "n": len(pnls), "win_rate": win_rate, "profit_factor": profit_factor,
        "avg_pnl_pct": mean_pnl, "sharpe_like": sharpe_like, "max_drawdown_pct": max_dd,
    }
...
def main():
    ...
    if hit_rate is not None:
        ...
        # [2차 리뷰 #9] 승률 외 손익비/Sharpe유사/MDD까지 대시보드로 출력
        stats = compute_performance_stats(reco_log)
        if stats:
            pf_txt = f"{stats['profit_factor']:.2f}" if stats['profit_factor'] != float('inf') else "∞(손실 없음)"
            print(f"   성과 대시보드(최근 {stats['n']}건): 승률 {stats['win_rate']*100:.0f}%, "
    ...
    # [2차 리뷰 #1 데이터 누수 방지] 여기서 로드하는 weights는 "지난 실행"이 끝나면서 저장해둔
    # 값이다 — 이번 실행에서 새로 학습되는 값이 이번 추천에 섞여 들어가는 일은 없다(아래에서
    # 이번 데이터로 다시 학습한 결과는 save_weights()로 저장만 하고 이번 rule_score 계산에는
    # 재사용하지 않음).
    weights = load_weights()
    weights_age = "기본값(저장된 학습 이력 없음)"
    if os.path.exists(WEIGHTS_STATE_PATH):
        mtime = datetime.fromtimestamp(os.path.getmtime(WEIGHTS_STATE_PATH))
        weights_age = f"{mtime.strftime('%m-%d %H:%M')}에 학습됨 (이번 실행 데이터 아님)"
    print(f"   앙상블 가중치 출처: {weights_age}")
    ...
    # ⑧ 이번에 모은 시계열로 obv/cmf/bb_macd/volume 가중치를 워크포워드로 재추정 — 저장만 하고
    #    이번 실행의 rule_score/추천에는 반영하지 않는다(다음 실행부터 적용, 데이터 누수 방지).
    new_weights = run_walk_forward_backtest(dfs_by_ticker, weights)
    save_weights(new_weights)
    print("   (재추정된 가중치는 다음 실행부터 적용됩니다)")
